...
export default function App() {
  ...
  return (
    <>
      <div className='py-2 dark:bg-slate-900 bg-white flex flex-col md:flex-row md:justify-between z-[60]'>
        <div className='z-[60]'>
 ...
 dark:border-slate-900 border-white border-r-2 hover:border-blue-500'>
      ...
      <div className='w-full min-h-screen dark:bg-slate-200 bg-slate-500 relative'>
        <div className='flex justify-center items-center absolute top-4 right-4 rounded-lg bg-white dark:bg-slate-900 overflow-hidden'>
          {options.map((opt) => (
            <button key={opt.theme} onClick={() => setTheme(opt.theme)} className={`w-6 h-6 m-1 rounded-lg text-black dark:text-white text-lg dark:hover:bg-slate-800 hover:bg-gray-200 ${theme === opt.theme ? 'dark:text-red-500 text-yellow-600' : ''}`}>
